...
if(isset($_REQUEST['id'])){
	...
	$sql_attencde = mysqli_query( $conn, "select * from cm_events_confirmed_delegates WHERE id=$attendence_id" );
  	$list_attendce = mysqli_fetch_array( $sql_attencde );
	$event_id = $list_attendce['event_id'];
	$email_id = $list_attendce['email'];
	
			$firstname =preg_replace('/[^A-Za-z0-9\-]/', '', $list_attendce['delegate_first_name']);
			$lastname =preg_replace('/[^A-Za-z0-9\-]/', '', $list_attendce['delegate_last_name']);
			$jobtitle =preg_replace('/[^A-Za-z0-9\-]/', '', $list_attendce['job_title']);
			$cleanedid = str_replace([' ', '+'], '', $list_attendce['phone_number']);		
			$cleanedNumber = str_replace([' ', '+'], '', $list_attendce['mobile_number']);
			$rw_company = strtolower(preg_replace('/[^a-z0-9]+/i', '',  $list_attendce['company_name']));
			$rw_street = strtolower(preg_replace('/[^a-z0-9]+/i', '',  $list_attendce['street_address']));
			$rw_city = strtolower(preg_replace('/[^a-z0-9]+/i', '',  $list_attendce['city']));
			$rw_country = strtolower(preg_replace('/[^a-z0-9]+/i', '',  $list_attendce['country_region']));
			$rw_postal = strtolower(preg_replace('/[^a-z0-9]+/i', '',  $list_attendce['postal_code']));
	
	$sql_event_core = mysqli_query( $conn, "select * from cmf_event WHERE ID='$event_id'" );
  	$list_event_core = mysqli_fetch_array( $sql_event_core );
	
	$sql_cmf_hubspot = mysqli_query( $conn, "select * from cmf_hubspot WHERE event_id='$event_id' and email='$email_id'" );
  	$list_hubspot = mysqli_fetch_array( $sql_cmf_hubspot );
	...
	<div class="modal-content">
		...
		<img src="Spin-1s-200px.gif">
          ...
          <div class="show-detail">
					 ...
					 <?php if(mysqli_num_rows($sql_cmf_hubspot) > 0){?>
				  	<table width="800px">
					  <thead>
						  <tr>
						 <th></th>
						...
						<tbody>
							...
							<tr>
								...
								<td><?php echo $list_attendce['mobile_number'];?></td>
							...
							<tr>
								...
								<td><?php echo $list_attendce['company_name'];?></td>
							...
							<tr>
								...
								<td><?php echo $list_attendce['street_address'];?></td>
							...
							<tr>
								...
								<td><?php echo $list_attendce['country_region'];?></td>

...
if ( $_SESSION[ 'id' ] ) {
  ...
  $login_access = mysqli_fetch_array( $login_sql );
	...
	if(isset($_REQUEST['edit'])){
		...
		if(isset($listrole_sql['cl_id'])){$listId = $listrole_sql['cl_id'];}else{$listId='';}
		$btn_sub = "roleupdate";
		$btn_txt = "Update";
	}
	else{
          ...
          <div class="card">
            ...
            <form method="post" action="configuration.php" enctype="multipart/form-data">
                ...
                <div class="col-md-12 mt-4" style="margin-bottom: 20px;">
                  <small id="label" class="form-text text-muted mb-2">Level</small>
					<select name="label" class="form-control" required>
						...
						<option value="1" <?php if(isset($_REQUEST['edit'])){ if($listrole_sql['level_num']=='1'){ echo "selected";}} ?>>1</option>
						<option value="2" <?php if(isset($_REQUEST['edit'])){if($listrole_sql['level_num']=='2'){ echo "selected";}} ?>>2</option>
						<option value="3" <?php if(isset($_REQUEST['edit'])){if($listrole_sql['level_num']=='3'){ echo "selected";}} ?>>3</option>
						<option value="4" <?php if(isset($_REQUEST['edit'])){if($listrole_sql['level_num']=='4'){ echo "selected";}} ?>>4</option>
					</select>
                  </div>
                  ...
                  <small id="wishlist_n" class="form-text text-muted  mb-2">From Wishlist (N)*</small>
					<input type="text" class="form-control" id="wishlist_n" name="wishlist_n" aria-describedby="From Wishlist (N)" value="<?php if(isset($_REQUEST['edit'])){ echo $listrole_sql['wishlist_n'];} ?>">
                  </div>
				  
				  <div class="col-md-12 mt-3">
                  <small id="wishlist_2" class="form-text text-muted mb-2">From Wishlist (2)*</small>
					<input type="text" class="form-control" id="wishlist_2" name="wishlist_2" aria-describedby="From Wishlist (2)" value="<?php if(isset($_REQUEST['edit'])){ echo $listrole_sql['wishlist_2'];} ?>">
                  </div>
				  
				  <div class="col-md-12 mt-3">
                  <small id="wishlist_3" class="form-text text-muted mb-2">From Wishlist (3)*</small>
					<input type="text" class="form-control" id="wishlist_3" name="wishlist_3" aria-describedby="From Wishlist (3)" value="<?php if(isset($_REQUEST['edit'])){ echo $listrole_sql['wishlist_3'];} ?>">
                  </div>
				  
				  <div class="col-md-12 mt-3">
                  <small id="not_wishlist" class="form-text text-muted mb-2">Not on Wishlist</small>
					<input type="text" class="form-control" id="not_wishlist" name="not_wishlist" aria-describedby="Not on Wishlist" value="<?php if(isset($_REQUEST['edit'])){ echo $listrole_sql['wishlist_no'];} ?>">
                  </div>
              ...
              <div class="modal-footer">
				  <input type="hidden" name="cl_id" value="<?php if(isset($_REQUEST['edit'])){ echo $listrole_sql['cl_id'];}?>">
				  <a href="configuration.php" type="button" class="btn btn-light">Back</a>

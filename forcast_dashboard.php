  ...
  <div class="page-wrapper">
    <div class="container-fluid">
      <div class="filter_form">
        <div class="row">
          <div class="col-md-11">
            <form action="forcast_dashboard.php" method="get">
              <h4>Filter</h4>
              <div class="row"> 
                <!--
								<div class="col-md-2">
...
-->
                ...
                <div class="decom_dh2">
                  ...
                  <select class="form-control eventdh" name="event">
                    ...
                    <?php
                    $QrEventData = mysqli_query( $conn, "select * from cmf_event WHERE STATUS='Completed' order by EVENT_TITLE ASC" );
                    while ( $RowQrEvent = mysqli_fetch_array( $QrEventData ) ) {
						...
						$id_to_encrypt1 = $RowQrEvent['ID'];
						$key = "16_byte_secret_key"; // AES requires a 16-byte key
						$encrypted_id = encrypt_id($id_to_encrypt1, $key);
                      ?>
                    <option value="<?php echo $encrypted_id;?>"><?php echo $RowQrEvent['EVENT_TITLE']." - (".$SGdate.")";?></option>
                    <?php } ?>
                  </select>
                </div>
                <div class="col-md-4">
        ...
        <div class="col-12" style="position:static">
          <div class="card" style="position:static">
            <div class="card-body">
				<div class="decom_dh4">	
			   <div class="decom_dh5">
					 <form action="cfm_dasboard_data.php" method="post">
              <?php
              ...
              <button type="submit" class="btn btn-orange" style="float: right; background-color: #fb8c00; font-size: 11px; margin-top: 10px; padding: 4px 6px; line-height: 1.1;float: left; border-radius: 5px;"><span style="font-size: 15px; font-weight: bold; ">Download</span><br>Mismatch Report</button>
            </form>
				 </div>
			  	<div class="decom_dh5">
					<form action="cfm_dasboard_commission_date.php" method="post">
              <?php
              if ( isset( $_REQUEST[ 'de' ] ) ) {
				 echo "<input type='hidden' name='de_request' value='".$_REQUEST[ 'de' ]."'>"; 
              }
              ...
              <button type="button" data-toggle="modal" data-target="#form" class="btn btn-orange" style="float: right; background-color: #0b4596; font-size: 12px; margin-top: 10px; padding: 4px 6px; line-height: 1.1;border: 0;color: #fff; border-radius: 5px; text-align: center;"><span style="font-size: 15px;font-weight: bold; ">Download</span><br>Commission Report</button>
            </form>
				 </div>
...
<!--                        <th>WISHLIST CATEGORY</th>-->
                      ...
                      <tbody>
                        <?php
//                        $sql2 = "SELECT * FROM cmf_de_commission
//					  INNER JOIN cm_events_confirmed_delegates ON cmf_de_commission.attend_id = cm_events_confirmed_delegates.id
//					  WHERE $search ORDER BY attend_id DESC";
						 
						  $sql2 = "SELECT * FROM cmf_de_commission
					  INNER JOIN cm_events_confirmed_delegates ON cmf_de_commission.attend_id = cm_events_confirmed_delegates.id
					   where event_id!='' $search  ORDER BY attend_id DESC";
                        $totalNetcom = '0';
                        $result2 = $conn->query( $sql2 );
                        if ( $result2->num_rows > 0 ) {
                          while ( $row2 = $result2->fetch_array() ) {
                            $event_id = $row2[ 'event_id' ];
                            $qrSql_event = mysqli_query( $conn, "select * from cmf_event WHERE ID='$event_id'" );
                        ...
                        <tr>
                          <td></td>
                          <td><?php echo $row2['delegate_first_name']." ".$row2['delegate_last_name'];?></td>
...
-->
                          ...
                          <td><?php
                          if ( $row2[ 'de_status' ] == 'PRESENT' && $row2[ 'level' ] && $row2[ 'valid' ]=='' ) {
                  ...
                  <div class="show-detail rt1" id="target_hit_content">
                    ...
                    <table>
                      ...
                      <th></th>
                        ...
                        $totalhit = '0';
                        $floorTtoal = '0';
//                        $total_delegate = mysqli_query( $conn, "select DISTINCT de,ev_id from cmf_de_commission 
//						INNER JOIN cm_events_confirmed_delegates ON cmf_de_commission.attend_id = cm_events_confirmed_delegates.id
//						WHERE $search_sec status='YES'" );
						
						 $total_delegate = mysqli_query( $conn, "select DISTINCT de,ev_id from cmf_de_commission 
						INNER JOIN cm_events_confirmed_delegates ON cmf_de_commission.attend_id = cm_events_confirmed_delegates.id WHERE $search_sec event_id!=''" );
						
                        while ( $list_delegate = mysqli_fetch_array( $total_delegate ) ) {
                          $same_de = $list_delegate[ 'de' ];
                          $evid = $list_delegate[ 'ev_id' ];

                          $numbof_del = mysqli_query( $conn, "select * from cmf_de_commission
						INNER JOIN cm_events_confirmed_delegates ON cmf_de_commission.attend_id = cm_events_confirmed_delegates.id
						WHERE de='$same_de' and ev_id='$evid'" );

                          $evid_del = mysqli_query( $conn, "select * from cmf_de_commission
						INNER JOIN cm_events_confirmed_delegates ON cmf_de_commission.attend_id = cm_events_confirmed_delegates.id
						WHERE ev_id='$evid' and de='$same_de' AND de_status='PRESENT' AND level != '' and valid=''" );

                          $eventDetail = mysqli_query( $conn, "select * from cmf_event WHERE id='$evid'" );
                          ...
                          $sql_target_hit = mysqli_query( $conn, "select * from cmf_target_hit WHERE eventId='$evid' and de='$same_de'" );
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header border-bottom-0">
        <h5 class="modal-title" id="exampleModalLabel">Download Commission Report</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form method="post" action="cfm_dasboard_commission_date.php">
        ...
        <div class="modal-footer border-top-0 d-flex justify-content-center">
          <button type="submit" name="report_filter" class="btn btn-primary">Submit</button>
        </div>
      </form>
    </div>
  </div>

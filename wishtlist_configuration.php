...
if ( $_SESSION[ 'id' ] ) {
	...
	if(isset($_REQUEST['rolesubmit'])){
		...
		$query000="insert into cm_wislist(level,commission,wishlist)
		values('$level','$commission','$wishlist')"; 
	...
	session_write_close();
	header('Location: wishtlist_configuration.php');
	}
	else if(isset($_REQUEST['roleupdate'])){
		echo $cl_id  = $_REQUEST['cl_id'];
		$level = $_REQUEST['label'];
		$commission = $_REQUEST['commission'];
		$wishlist = $_REQUEST['wishlist'];
		$query000="update cm_wislist SET level='$level',commission='$commission',wishlist='$wishlist' WHERE id =$cl_id";
		mysqli_query($conn,$query000);
		$errmsg_arr[] = '<div class="alert alert-danger alert-dismissable"> <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a> <strong>Success!</strong> Data Add Sucessfully.</div>';
	$errflag = true;
	$_SESSION['register_success'] = $errmsg_arr;
...
{
$encrypted_delete_id=$_REQUEST['del'];
$key = "16_byte_secret_key";
$cl_id400 = decrypt_id($encrypted_delete_id, $key);
$query="delete from cm_wislist WHERE id=$cl_id400";
                                 ...
                                 <div class="row">
                                    ...
                                    <div class="col-6">
                                        ...
                                        <div class="col-6">
                                            ...
                                            <a type="button" class="btn btn-info btn-sm" href="wishtlist_configuration_form.php" style="float: right;"><i class="fas fa-plus"></i> Create Configuration</a>
...
<div onscroll='scroller("scrollme", "scroller")' id="scrollme">
  <table id="multi_col_order" width="1000">
     ...
     <tbody>
                     ...
                     <?php $sql2 = "SELECT * FROM cm_wislist ORDER BY id ASC";
                   ...
                   <tr>
                                        ...
                                        <td></td>
                                        <td id="status"><?php echo $row2['level'];?></td>

					   					<td id="status"><?php echo $row2['wishlist'];?></td>
					   					<td id="status"><?php echo $row2['commission'];?></td>
					   				
                                        
                                        <td id="status">
											<?php
							
							$id_to_encrypt1 = $row2['id'];
$key = "16_byte_secret_key"; // AES requires a 16-byte key
$encrypted_id = encrypt_id($id_to_encrypt1, $key);
$edit_url = "wishtlist_configuration_form.php?edit=" . urlencode($encrypted_id);
$delete_url = "wishtlist_configuration.php?del=" . urlencode($encrypted_id);
                        ?>
											<a href="<?php echo $edit_url;?>" class="btn btn-info btn-sm">Edit</a>
											
											<a href="<?php echo $delete_url;?>" class="btn btn-danger btn-sm" onClick="return confirm('Do you really want to remove it?')">Delete</a>
					   </td>
                                    </tr>
